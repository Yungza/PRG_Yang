    ...
    internal class Program
    {
        ...
        class Person
        {
            ...
            public void SaveToTxt()
            {
                ...
                toWrite += Height + "\n";
                toWrite += Weight;
                string filePath = "U:\\AAAPRG\\PRG_Yang\\Files\\" + Name + ".txt";
            ...
            public static Person LoadFromTxt()
            {
                string name = Console.ReadLine();
                string filePath = "U:\\AAAPRG\\PRG_Yang\\Files\\" + name + ".txt";
                if(File.Exists("U:\\AAAPRG\\PRG_Yang\\Files\\" + name + ".txt"))
                {
                    using (StreamReader reader = new StreamReader(filePath))
                    {
                        return new Person(reader.ReadLine(), float.Parse(reader.ReadLine()), float.Parse(reader.ReadLine()));
                    }
                }
                else
                {
            ...
            public void SaveToCsv()
            {
                ...
                toWrite += Height + "\n";
                toWrite += Weight;
                string filePath = "U:\\AAAPRG\\PRG_Yang\\Files\\" + Name + ".csv";
                using (StreamWriter writer = new StreamWriter(filePath))
                {
                    writer.Write(toWrite);
                }
            ...
            public static Person LoadFromCsv()
            {
                ...
                if (File.Exists(filePath))
                {
                    using (StreamReader reader = new StreamReader(filePath))
                    {
                        string beforeArray = reader.ReadLine();
                        string ě = reader.ReadLine();
                        string š = reader.ReadLine();
                        string[] array = beforeArray.Split(',');
                        return new Person(array[0], float.Parse(array[1]), float.Parse(array[2]));
                    }
                }
